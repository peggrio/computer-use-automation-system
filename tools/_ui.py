"""Internal scripted adapter walkthrough used by browser and safety tests."""
import argparse
import asyncio
import json
import os
...
async def walkthrough(adapter):
    """Run a fixed test scenario with adapter methods, not the replay interpreter."""
...
async def main(args):
    ...
    async with BrowserAdapter(
        capability, profile, {'account_id': args.account, 'transaction_id': args.transaction},
        headless=not args.headed, timezone_id=args.timezone, evidence_root=args.evidence_root,
    ) as adapter:
        await adapter.login(
            os.environ.get('PARABANK_USERNAME', 'john'),
            os.environ.get('PARABANK_PASSWORD', 'demo'),
        )
        ...
        print(json.dumps({
            'check': 'scripted_adapter_smoke', 'status': result['status'],
